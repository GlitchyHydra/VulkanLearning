...
{
	template<>
	struct hash<Vipera::Model::Vertex>
	{
		size_t operator()(Vipera::Model::Vertex const& vertex) const
		{
			size_t seed = 0;
			Vipera::hashCombine(seed, vertex.position, vertex.color, vertex.normal, vertex.uv);
			return seed;
		}
	};
}
